...
@section('url','Sửa tài khoản: '.$users->name)
@section('main')
<form action="" method="POST" enctype="multipart/form-data" role="form" >
        ...
        <div class="form-group m-3">
            ...
            <input type="text" class="form-control" name="name" id='name' placeholder="Nhập tên người dùng" value="{{ $users->name }}">
            <!--Gọi validate -->
            @if($errors->has('name'))
                {{$errors->first('name')}}
            @endif
        ...
        <div class="form-group m-3">
            <div class="mb-3">
                <img src="{{ asset('images/'.$users->images) }}" alt="Ảnh" style="width: 200px; height:150px;">
            </div>
        ...
        <div class="form-group m-3">
            ...
            <input type="email" class="form-control" name="email" id='email' placeholder="Nhập email" value="{{ $users->email }}">
            <!--Gọi validate -->
            @if($errors->has('email'))
                {{$errors->first('email')}}
            @endif
